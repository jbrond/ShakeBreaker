...
-(void) countdownSpeak
{
    [fliteEngine speakNowText:@"1    2    3    Go"];
}
    ...
    if (gameRunning) {
        if (gameTime>=MAX_GAME_TIME || allDone) {
            ...
            if (allDone) {
                ...
                //sum the time
                for (int n=1;n<7;n++) {
                    if (positionGameTime[n]>tid)
                        tid = (float)positionGameTime[n];
                }
            
                tid = tid * GAME_TIME_RESOLUTION;
                
                gameEndMessage = [NSString stringWithFormat:@"Game over time %2.1f seconds",tid];
            }
...
- (IBAction)sendShakeCommand:(id)sender {
    ...
    [self registerShakeSound];
    
    sleep(1);
    
    [self unRegisterShakeSound];
}
